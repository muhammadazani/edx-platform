...
class ConditionalDescriptor(ConditionalFields, SequenceDescriptor):
    ...
    def definition_from_xml(cls, xml_object, system):
        fields = {}
        # Convert sources attribute to a ReferenceList field type so Location/Locator
        # substitution can be done.
        if 'sources' in xml_object.attrib and isinstance(xml_object.attrib['sources'], basestring):
            sources = ConditionalDescriptor.parse_sources(xml_object.attrib)
            fields['sources_list'] = sources
        children = []
        show_tag_list = []
        for child in xml_object:
            if child.tag == 'show':
                ...
                for location in locations:
                    ...
                    show_tag_list.append(location)  # pylint: disable=no-member
            else:
                try:
        ...
        fields['show_tag_list'] = show_tag_list
        return fields, children

    def definition_to_xml(self, resource_fs):
        def to_string(list):
            """ Convert List of stings to a single string with "; " as the separator. """
